...
/*   Updated: 2019/07/29 19:12:14 by crfernan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_get_parameters(t_master *mstr, char *line, int nb_line)
{
	if (nb_line == 0)
	{
		...
		return (mstr->nb_ants != 0 ? TRUE : FALSE);
	}
	else if (nb_line == 1)
	{
		mstr->nb_nodes = ft_atoi(line + 31);
		return (mstr->nb_nodes != 0 ? TRUE : FALSE);
	}
	else if (nb_line == 2)
	{
		mstr->nb_pipes = ft_atoi(line + 31);
		return (mstr->nb_pipes != 0 ? TRUE : FALSE);
	}
	else if (nb_line == 3)
	{
	...
	else if (nb_line == 4)
	{
		...
		return (mstr->nb_movements != 0 ? TRUE : FALSE);
	}
	return (FALSE);
}
